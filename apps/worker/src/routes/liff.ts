...
liffRoutes.get("/auth/line", async (c) => {
  ...
  const isInAppBrowser =
    /twitter|twitterandroid|\b(fbav|fban|instagram|line\/|micromessenger)\b/i.test(
      ua,
    );
        ...
        if (
          ...
          if (!existing) {
            ...
            if (firstStep && firstStep.delay_minutes === 0) {
              const { resolveMetadata: resolveMetaLiff } =
                await import("../services/step-delivery.js");
...
async function applyXHarnessActions(
  ...
  if (result.scenarioId) {
    try {
      ...
      console.log(
        `X Harness: enrolled friend ${friendId} in scenario ${result.scenarioId}`,
      );
    } catch (err) {
