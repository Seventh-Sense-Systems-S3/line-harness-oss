...
import { handleMizukagamiMessage } from "../services/mizukagami.js";
...
/**
 * Forward text message to Supabase auto_action_queue (best-effort).
 * Failure does not affect the LINE response or D1 processing.
 */
async function forwardToSupabase(
  ...
  try {
    const res = await fetch(`${env.SUPABASE_URL}/rest/v1/auto_action_queue`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      console.error(
        `[supabase-bus] Insert error: ${res.status} ${await res.text()}`,
      );
    } else {
      console.log(`[supabase-bus] Queued message from ${userId}`);
    }
  } catch (err) {
    console.error("[supabase-bus] Forward failed (best-effort):", err);
  }
}
...
async function handleEvent(
  ...
  if (event.type === "message" && event.message.type === "text") {
    ...
    if (!friend) {
      // Auto-upsert: existing followers who sent messages before D1 migration
      let profile;
      try {
        profile = await lineClient.getProfile(userId);
      } catch (err) {
        console.error("Failed to get profile for auto-upsert", userId, err);
      }
      ...
      console.log(`[webhook] Auto-upserted friend ${userId} → ${friend.id}`);
    }
    ...
    // Supabase auto_action_queue へベストエフォート転送（ADR-031: line-harness-oss Bus統合）
    if (env) {
      forwardToSupabase(env, incomingText, userId);
    }

    // MIZUKAGAMI 診断フロー（auto_replies より優先）
    try {
      const handled = await handleMizukagamiMessage(
        db,
        lineClient,
        userId,
        incomingText,
      ...
      console.error("[mizukagami] Handler error:", err);
    }
