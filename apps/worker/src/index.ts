...
export type Env = {
  Bindings: {
    ...
    // ─── Mizukagami integration (Channel-Based Routing) ───
    MIZUKAGAMI_WORKER_URL: string;
    MIZUKAGAMI_API_KEY: string;
    MIZUKAGAMI: Fetcher;
    MIZUKAGAMI_DEV?: Fetcher; // DEV LINE OA → DEV mizukagami worker (Service binding)
    MIZUKAGAMI_DEV_LINE_CHANNEL_ID?: string;
    // ─── Notification / Supabase notify ───
...
app.get("/r/:ref/help", (c) => {
  ...
  const browserName = isIOS
    ...
    : isAndroid
      ? "Chrome"
      : "ブラウザ（iPhoneは Safari／Androidは Chrome）";
...
async function scheduled(
  ...
  const { recoverStalledBroadcasts, recoverStuckDeliveries } =
  ...
  try {
    ...
    if (result.sent + result.failed > 0) {
      console.log(
        `[booking-reminders] sent=${result.sent} failed=${result.failed}`,
      );
    }
  } catch (e) {
  ...
  try {
    ...
    if (result.sent + result.failed > 0) {
      console.log(
        `[event-booking-reminders] sent=${result.sent} failed=${result.failed}`,
      );
    }
  } catch (e) {
